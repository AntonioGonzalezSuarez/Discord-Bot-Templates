...
class CustomBot(BaseBot):
    def __init__(self, discord_token: str = os.getenv("DISCORD_TOKEN")):
        ...
        async def help(ctx):
            ...
            await ctx.send("Current available functionalities: \n"
                           "- /help\n"
                           "- /info")
        ...
        async def on_ready():
            ...
            print(f"At {datetime.now()} bot is connected to {len(self.guilds)} discord servers")

        @self.bot.event
        async def on_guild_join(guild):
            for channel in guild.channels:
                if channel.type.name == "text":
                    await self.bot.get_channel(channel.id).send("Hello my friends. I am a bot desiged by Toni Tonin. "
                                                                "To get more info write !help")
            self.update_guilds()
            print(f"At {datetime.now()} bot is connected to new server. Giving service to: {len(self.guilds)} discord "
                  f"servers")

        @self.bot.event
        async def on_guild_remove(guild):
            del self.guilds[guild.id]
            print(f"At {datetime.now()} bot is disconnected from {guild.name}. Giving service to: {len(self.guilds)}"
                  f" discord servers")
    def start(self):
        ...
        for guild in self.bot.guilds:
            ...
            for channel in guild.channels:
                if channel.type.name != "category":
                    ...
                    self.guilds[guild.id]["channels"][channel.id]["channel_name"] = channel.name
                    self.guilds[guild.id]["channels"][channel.id]["channel_type"] = channel.type.name
